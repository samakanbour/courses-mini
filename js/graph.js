document.addEventListener('DOMContentLoaded', function () {
	...
	Tabletop.init({
		...
		callback: function(data, tabletop) { init(data, 1) }
	});

	Tabletop.init({
		key: "0AhtG6Yl2-hiRdFhTS3h1SnFZY09LRm0tSkdGR2ZWZ2c",
		callback: function(data, tabletop) { init(data, 2) }
	});

	$("#course-planner #info").tooltip({placement: 'right', title: 'Click and hold on a course to highlight dependencies, double-click for more information'});
	$("#course-planner .colors").tooltip({placement: 'right', title: 'Click to view the color legend'});
	$("#course-planner .colors").click(function(e){
		var num = e.target.id.split('-')[1];
		if ($("#course-planner #section-" + num)[0].style.visibility == "visible"){
			$("#course-planner #section-" + num).css("visibility","hidden");
		} else {
			$("#course-planner #section-" + num).css("visibility","visible");
		}
	});
	$("#course-planner article").click(function(e){
		if (e.target.className == "colors"){ return }
		$("#course-planner section").css("visibility","hidden");
	});
});
...
function init(result, id) {
	...
	result.colors.elements.forEach(function (row) {
		...
		$('#legend-' + id).append("<li><span style='background:" + row.color + ";'></span>" + row.legend + "</li>"); 
	});
    ...
    up = function () {
    	...
    	if (this.type == 'text') {
			shape = this.pair;
    		 ...
    		 if (shape.attr("cy") <=  62.5 + 30) { reposition(shape,  25 + 30); } 
    	else if (shape.attr("cy") <= 137.5 + 30) { reposition(shape, 100 + 30); }
    	else if (shape.attr("cy") <= 212.5 + 30) { reposition(shape, 175 + 30); }
    	else if (shape.attr("cy") <= 287.5 + 30) { reposition(shape, 250 + 30); }
    	else if (shape.attr("cy") <= 362.5 + 30) { reposition(shape, 325 + 30); }
    	else if (shape.attr("cy") <= 437.5 + 30) { reposition(shape, 400 + 30); }
    	else if (shape.attr("cy") <= 512.5 + 30) { reposition(shape, 475 + 30); }
    	else { reposition(shape, 550 + 30); }
    	unselectCourse();
    }
...
Raphael.el.tooltip = function (text) {
	...
	this.hover( function(event) {
		...
		if (position > 350) { direction = 'left'; position = shape.attr('cx') - 33 - shape.tooltitle.getBBox().width/2; }
		if (direction == 'right') {
		...
		if (direction == 'top') {
			shape.tooltitle.attr({x: position, y: shape.attr('cy') - 40});
		} else {
